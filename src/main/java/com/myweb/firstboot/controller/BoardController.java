...
public class BoardController { //게시판 컨트롤러
	...
	public String boardPage(Model model, @PathVariable("board_no") int board_no) {
		//게시판 리스트
		List<PostDto> list = service.getPostListByBoard(board_no);
	...
	public String galaryPost(Model model, PostDto dto, 
			@RequestParam(value="file", required=false) MultipartFile[] files) { //파일 가져오는 클래스 적용해서 첨부파일 기능 적용 // 여러 파일 적용하기 위해 배열로 변경
		...
		if (files != null) {
			for (MultipartFile file : files) { // 파일 개수만큼 돌려서 file로 넣어줌
				try {
					...
					// 실제 파일 저장위치에 파일 업로드
					file.transferTo(new File(path + file_name)); // 오브젝트로 넘기기
					fileService.imgUpload(imgDto);
				} catch (IOException e) {
					e.printStackTrace();
				}
			}
			List<GalaryDto> galary_list = fileService.ImgDownloadList(dto.getUserid());
			model.addAttribute("galary_list", galary_list);
		}
		return "galaryList";
	}
}
